...
def make_counter():
    ...
    def counter(string):
        nonlocal num_called
        if string not in num_called:
            num_called[string] = 1
        else:
            num_called[string] += 1
        return num_called[string]
    return counter
...
class VendingMachine:
    ...
    def vend(self):
        abs_change = abs(self.price - self.balance)
        if self.stock <= 0:
            return 'Machine is out of stock.'
        elif self.balance < self.price:
            return 'You must deposit $' + str(abs_change) + ' more.'
        else:
            self.balance = 0
            self.stock -= 1
            text = 'Here is your ' + self.name
            if abs_change:
                text += ' and $' + str(abs_change) + ' change.'
            else:
                text += '.'
            return text
...
def preorder(t):
    ...
    if t.is_leaf():
        ...
        return flatten([t.label] + [preorder(branch) for branch in t.branches])
