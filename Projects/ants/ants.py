...
def damage_calc(self, damage_amount):
    lst = self.place.bees
    for i in lst[:]:
        Insect.reduce_armor(i, damage_amount)
...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            elif insect.is_container == True and self.ant.is_container == False:
...
class Bee(Insect):
    ...
    #The following constructor was created for the EC to work
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive, min_range=0, max_range=float('inf')):
        ...
        while current_place != beehive and counter <= max_range:
            potential_bee = random_or_none(current_place.bees)
            if potential_bee:
                return potential_bee
            counter += 1
            current_place = current_place.entrance
        return None
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, beehive):
        ...
        This method returns None if there is no such Bee (or none in range).
        """
        return ThrowerAnt.nearest_bee(self, beehive, max_range = self.max_range)
    # END Problem 4
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, beehive):
        ...
        This method returns None if there is no such Bee (or none in range).
        """
        return ThrowerAnt.nearest_bee(self, beehive, self.min_range)
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        damage_calc(self, damage_amount)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            potential_bee = random_or_none(self.place.bees)
            if potential_bee:
                self.eat_bee(potential_bee)
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        damage_calc(self, self.damage)
        # END Problem 7

# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt does nothing each turn. It is useful because it has a large armor value."""
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if not self.contained_ant and not other.is_container:
            return True
        return False
        # END Problem 9
...
# END Problem 13
        ...
        if QueenAnt.num_queen > 1:
            ...
            while current_place:
                if current_place.ant:
                    if current_place.ant not in QueenAnt.ants_buffed:
                        current_place.ant.damage *= 2
                        QueenAnt.ants_buffed += [current_place.ant]
                    # current ant is a container and it contains an ant which has not been buffed yet.
                    if current_place.ant.is_container and current_place.ant.contained_ant \
                       and current_place.ant.contained_ant not in QueenAnt.ants_buffed:
                        current_place.ant.contained_ant.damage *= 2
                        QueenAnt.ants_buffed += [current_place.ant.contained_ant]
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 13
        if QueenAnt.num_queen == 1 and self.armor <= amount:
            bees_win()
        else:
            Insect.reduce_armor(self, amount)
        # END Problem 13
...
def make_scare(action, bee):
    ...
    # BEGIN Problem EC
    def new_action(colony):
        if not bee.scared_once:
            bee.scared_once = True
            bee.move_forward = False
            action(colony)
        else:
            action(colony)
    return new_action
...
def apply_effect(effect, bee, duration):
    ...
    def counted_action(colony):
        nonlocal duration
        if duration > 0:
            duration -= 1
            effect(original_action, bee)(colony)
        else:
            bee.move_forward = True
            #bee.action = original_action           ######???### Why having this line makes the test fail? Please comment.
            original_action(colony)
    bee.action = counted_action
