...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if not isinstance(operator, MacroProcedure):
            return scheme_apply(operator, rest.map(lambda elem: scheme_eval(elem, env)), env)
        # END PROBLEM 4
        # BEGIN PROBLEM 20
        else:
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError("number of formals do not match that of the values")
        return child_frame
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('incorrect number of arguments: {0}'.format(repl_str(e)))
        # END PROBLEM 3
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    child_frame = env.make_child_frame(nil, nil)
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        check_formals(Pair(bindings.first.first, nil))
        child_frame.define(bindings.first.first, scheme_eval(bindings.first.rest.first, env))
