...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        temp = scheme_eval(first, env)
        check_procedure(temp)
        if not isinstance(temp, MacroProcedure):
            return scheme_apply(temp, rest.map(lambda x: scheme_eval(x, env)), env)
        else:   # EC Problem
            return scheme_eval(temp.apply_macro(rest, env), env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    temp = expressions.first
    if scheme_symbolp(temp):
        check_form(expressions, 2, 2)
        # BEGIN PROBLEM 5
        env.define(temp, scheme_eval(expressions.rest.first, env))
        return temp
        # END PROBLEM 5
    elif isinstance(temp, Pair) and scheme_symbolp(temp.first):
        # BEGIN PROBLEM 9
        vals = Pair(temp.rest, expressions.rest)
        env.define(temp.first, do_lambda_form(vals, env))
        return temp.first
        # END PROBLEM 9
    else:
        bad_val = temp.first if isinstance(temp, Pair) else temp
        raise SchemeError('non-symbol: {0}'.format(bad_val))
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 13
            if len(clause.rest) > 1:
                return eval_all(clause.rest, env)
            elif len(clause.rest) == 0:
                return test
            else:
...
def make_let_frame(bindings, env):
    ...
    for _ in range(len(bindings)):
        if len(bindings.first) != 2:
            raise SchemeError("bad definition")
        check_formals(Pair(bindings.first.first,nil))
        let_env.define(bindings.first.first, scheme_eval(bindings.first.rest.first,env))
...
def do_define_macro(expressions, env):
    ...
    if isinstance(expressions.first, Pair) and scheme_symbolp(expressions.first.first):
        macro_procedure = MacroProcedure(expressions.first.rest, expressions.rest, env)
        env.define(expressions.first.first, macro_procedure)
        return expressions.first.first
    else:
        null_val = expressions.first.first if isinstance(expressions.first, Pair) else expressions.first
        raise SchemeError('non-symbol: {0}'.format(null_val))
    # END Problem 20
