...
def distance(city1, city2):
    ...
    5.0
    """
    return sqrt((get_lon(city2) - get_lon(city1))**2
                + (get_lat(city2) - get_lat(city1))**2)
...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    return get_name(min(city1, city2, key = lambda x:
                        distance(x, make_city('PlaceX', lat, lon))))
...
def acorn_finder(t):
    ...
    if label(t) == 'acorn':
        return True
    elif is_leaf(t) or not is_tree(branches(t)):
        return False
    else:
...
def sprout_leaves(t, vals):
    ...
    """
    if is_leaf(t):
        return tree(label(t), [tree(vals[i]) for i in range(len(vals))])
    else:
